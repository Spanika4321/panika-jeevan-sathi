 ...
 *
 * No SITE_URL? A real local server is booted automatically — that is the CI
 * default. CI also ships scripts/resolve-prod-url.mjs, which auto-detects the
 * deployed production URL (Render API / PJS_PRODUCTION_URL) so nothing has to
 * be set by hand.
 *
 * Covers: loading, navigation, link/asset crawl (broken links), signup (+email
 * verification), login, session persistence across refresh, profile
 * create/edit, photo upload, search with filters, interest → accept, two-way
 * messaging, shortlist, notifications, logout/login, contact form, error
 * handling (404 UI, API 404, wrong password, short password, duplicate email)
 * and mobile responsive behaviour. Console errors, failed API requests and
 * runtime exceptions are collected on every page and fail the run.
 *
...
async function main() {
  ...
  try {
    ...
    /* 2b ─ broken links & missing assets */
    section('2b. Internal links & assets (crawl)');
    const links = new Map(); // url path -> page it was found on
    const origin = new URL(baseUrl).origin;
    for (const crawlPage of [
      '/',
      '/about.html',
      '/contact.html',
      '/terms.html',
      ...
      try {
        await pageA.goto(baseUrl + crawlPage, { waitUntil: 'domcontentloaded', timeout: 30000 });
      } catch {
        continue; // page may do a redirect chain — covered elsewhere
      }
      // Member pages client-side redirect to /login.html?next=… right after
      // DOMContentLoaded. Let the dust settle, then read the DOM (retry once
      // if a navigation is still in flight — the login page is equally valid).
      let refs = [];
      for (let attempt = 0; attempt < 3; attempt += 1) {
        await sleep(400);
        try {
          refs = await pageA.evaluate(() => {
            const out = [];
            for (const el of document.querySelectorAll('a[href], img[src], link[href], script[src]')) {
              out.push(el.getAttribute('href') || el.getAttribute('src'));
            }
            return out;
          });
          break;
      ...
      for (const rawRef of refs) {
        ...
        let target;
        try {
          target = new URL(rawRef, baseUrl + crawlPage);
        } catch {
          continue; // malformed href (mailto:, javascript:…)
        }
        if (target.origin !== origin) continue; // external (WhatsApp, Product Hunt…) — not crawled
        const key = target.pathname + target.search;
        if (!links.has(key)) links.set(key, crawlPage);
    ...
    let okLinks = 0;
    const brokenLinks = [];
    for (const [targetPath, fromPage] of links) {
      try {
        ...
        if (res.status < 400) okLinks += 1;
        else brokenLinks.push(`${targetPath} → HTTP ${res.status} (found on ${fromPage})`);
      } catch {
        brokenLinks.push(`${targetPath} → fetch failed (found on ${fromPage})`);
      }
    }
    check(
      `link/asset crawl: ${okLinks} reachable of ${links.size} internal references`,
      brokenLinks.length === 0,
      brokenLinks.slice(0, 6).join(' | ')
    );
  ...
  try {
    const lines = [
      ...
      '',
      '## Checks performed in every step',
      '',
      '- Page loading: every navigation waits for its content before continuing',
      '- Console errors: `pageerror` and `console.error` collected on every page',
      '- Failed API requests / broken responses: any HTTP >= 400 is flagged',
      '- Broken links & missing assets: internal link/asset crawl of the main pages',
      '- Runtime errors: any JS exception aborts the step and is reported',
      '- Mobile responsiveness: 390×844 viewport, horizontal-overflow checks, member bottom-nav',
      ''
    ];
    if (failures.length) {
      lines.push('## Failures', '');
      for (const f of failures) lines.push(`1. ${f}`);
      lines.push('');
    }
    ...
    console.log('  report: browser-tests/report.md');
  } catch (_) {
