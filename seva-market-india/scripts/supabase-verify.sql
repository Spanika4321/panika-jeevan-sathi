...
--   1. five rows, present = t                           — the tables exist
...
--   4. can_select = f and can_insert = f on all 10 rows — the REVOKEs landed
...
-- 1. Which of the five tables exist? A name missing here = that paste did not run.
select 'seva_mirror' as table_name, to_regclass('public.seva_mirror') is not null as present union all select 'seva_users', to_regclass('public.seva_users') is not null union all select 'seva_leads', to_regclass('public.seva_leads') is not null union all select 'seva_audit_logs', to_regclass('public.seva_audit_logs') is not null union all select 'seva_account_tokens', to_regclass('public.seva_account_tokens') is not null order by 1;

-- 2. Row Level Security must be ON for all five.
select relname as table_name, relrowsecurity as rls_enabled from pg_class where relnamespace = 'public'::regnamespace and relkind = 'r' and relname in ('seva_mirror','seva_users','seva_leads','seva_audit_logs','seva_account_tokens') order by 1;

-- 3. And no policy may exist: RLS with zero policies is what hides the rows.
select count(*) as policy_count from pg_policy p join pg_class c on c.oid = p.polrelid where c.relnamespace = 'public'::regnamespace and c.relname in ('seva_mirror','seva_users','seva_leads','seva_audit_logs','seva_account_tokens');

-- 4. The public keys must hold no table privilege. A server without those roles reads null.
select g.grantee, c.relname as table_name, case when pr.rolname is not null then has_table_privilege(pr.oid, c.oid, 'select') end as can_select, case when pr.rolname is not null then has_table_privilege(pr.oid, c.oid, 'insert') end as can_insert from pg_class c cross join (values ('anon'), ('authenticated')) as g(grantee) left join pg_roles pr on pr.rolname = g.grantee where c.relnamespace = 'public'::regnamespace and c.relkind = 'r' and c.relname in ('seva_mirror','seva_users','seva_leads','seva_audit_logs','seva_account_tokens') order by 1, 2;
...
-- 7. The durable tables stay empty until a real signup, enquiry or link arrives.
select (select count(*) from public.seva_leads) as leads_stored, (select count(*) from public.seva_users) as accounts_stored, (select count(*) from public.seva_audit_logs) as audit_rows_stored, (select count(*) from public.seva_account_tokens) as tokens_stored;
