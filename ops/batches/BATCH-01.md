...
**Protocol:** arena-termux-batch/1 · **Issued:** 2026-08-31T23:45:00Z · **By:** arena-coordinator
**Execute on:** termux · **Base commit:** `0a273a27e24a9efdbe9c275b4fd948191eded2ae` · **Branch:** `main`

## Why this batch

- Arena already executed every local verifier in its own coordinator sandbox (Linux, Node v22.22.3) BEFORE the Priti AI change, and all were green: check-syntax 41 files / 0 errors, health-check 95 passed 0 failed, e2e-test 134 passed 0 failed (default store AND PJS_STORAGE=json), agent-team-check PASS, agent-storage doctor PASS with a 12-entry intact hash chain, worktree clean. This batch is about proving the same on the preserved current tree.
...
- Recut after owner handoff: the 143-line Priti AI change in public/assets/js/app.js is intentional application work and is PRESERVED. It is the new approved UI baseline.
- The old fingerprint 5469d2287361… was the pre-Priti main. BATCH-01 no longer pins it as the current truth; T-01 captures the real device fingerprint and regenerates reports/ui-baseline-body.md5 from the preserved tree.
- This batch is read-only against public/**. The only tracked file that may differ from base is public/assets/js/app.js; T-02 and T-10 assert that and nothing else.
- no D1 write, no migration, no deploy, no git push, no destructive git command is attempted or allowed by this batch.
...
## T-01 — Approved UI baseline capture + device capability truth

| Field | Value |
| --- | --- |
| 1. Task ID | `T-01` (order 1) |
| 2. Assigned worker | manager — Manager (coordinator on-device) |
| 3. Exact objective | Record the device's real capability record AND capture the approved UI baseline: Node version, git/curl, free disk, HEAD/branch, worktree, and the ACTUAL sha256 fingerprint of the preserved public/ tree (which includes the approved Priti AI change). The command writes ops/batches/BATCH-01.baseline.json and regenerates reports/ui-baseline-body.md5 from the current tree. It never modifies, deletes, restores or cleans any public/** file. |
| 4. Allowed | actions: capture the current public UI baseline under explicit owner approval; read files; run read-only commands; print versions · files: `reports/agents/batch-01-t-01.evidence.md`, `ops/batches/BATCH-01.baseline.json`, `reports/ui-baseline-body.md5` |
| 4b. Forbidden | installing packages; editing any tracked file; network writes of any kind; any write under public/ |
| 5. Verification | exit 0 AND the output says the approved UI baseline was captured, names the baseline file ops/batches/BATCH-01.baseline.json, confirms reports/ui-baseline-body.md5 was regenerated, and confirms public/** was untouched. The new fingerprint is the real sha256 of the preserved device tree, NOT a value Arena guessed. |
| 6. Expected report | The captured fingerprint + file count, the baseline file path, and an explicit 'public/** untouched' statement. Also report node version, git/curl presence, disk, HEAD/branch and worktree state. |
| 7. Stop condition | If the command cannot write the baseline file (e.g. read-only ops/batches/), record BLOCKED naming the exact path/permission. If public/** is reported as modified by the baseline command, STOP: the tool has a bug and must not continue. |
| Needs | `node`, `git` |

```bash
$ node scripts/termux-batch.mjs baseline BATCH-01
```

## T-02 — Tree integrity at the approved app state

| Field | Value |
| --- | --- |
| 1. Task ID | `T-02` (order 2) |
| 2. Assigned worker | guardian — Guardian (Sardar) — safety & health authority |
| 3. Exact objective | Verify the app code on the device is exactly the approved state. Two separate claims: (a) under server.js, lib/, public/ and agents/ the ONLY tracked working-tree change is public/assets/js/app.js (the approved 143-line Priti AI feature); nothing else in app code is locally modified (untracked backups are ignored by --untracked-files=no so they are not treated as app code; scripts/ is tooling and outside this app-code pin); (b) committed runtime code between base 0a273a27e24a9efdbe9c275b4fd948191eded2ae and HEAD is byte-identical, so tooling commits can never mask or fake app-code drift. Syntax and the agent-team safety config must both be green. The whitespace gate is scoped to code/tooling paths on purpose. |
| 4. Allowed | actions: read files; run the repo's own syntax + team checks; verify the single approved app-code change · files: `reports/agents/batch-01-t-02.evidence.md` |
| 4b. Forbidden | editing any file to make a check pass; git checkout/reset/clean/stash; any write outside reports/; touching public/ |
| 5. Verification | check-syntax prints '41 checked, 0 with syntax errors'; agent-team-check prints 'AGENT TEAM CHECK: PASS'; app-diff-verify prints 'APP CODE DIFF OK' and exits 0; and the committed runtime diff + whitespace gate each print NOTHING. app-diff-verify is the machine check that the only allowed tracked app-code change is public/assets/js/app.js (the approved Priti AI feature). |
| 6. Expected report | The three PASS lines, the exact output of app-diff-verify (normally the single approved public/assets/js/app.js line), and confirmation the two committed checks printed nothing. |
| 7. Stop condition | If app-diff-verify names any file OTHER than public/assets/js/app.js, or the committed runtime diff prints a file: STOP the batch, paste `git status --porcelain --untracked-files=no -- server.js lib public agents` and `git log --oneline -3`, and wait for Arena. Never 'fix' it by editing, stashing, checking out or resetting. |

```bash
$ node scripts/check-syntax.mjs
$ node scripts/agent-team-check.mjs
$ node scripts/app-diff-verify.mjs
$ git diff --name-only 0a273a27e24a9efdbe9c275b4fd948191eded2ae HEAD -- server.js lib public agents
$ git diff --check 0a273a27e24a9efdbe9c275b4fd948191eded2ae HEAD -- server.js lib public agents scripts package.json
```
...
| 2. Assigned worker | manager — Manager (coordinator on-device) |
| 3. Exact objective | After all tests, prove no member data or throwaway database can leak into git and that the working copy holds no stray runtime artifacts: data/ and uploads/ stay ignored and untracked, the batch itself dirties only reports/ and ops/batches/ plus the one approved public/assets/js/app.js change, and no data/ directory was created inside the repo by T-03/T-04/T-05. The data-safety empties are scoped to data/, uploads/, storage/, server.js, lib/, agents/ — they deliberately exclude public/, whose single approved app.js change is asserted separately by app-diff-verify. |
| 4. Allowed | actions: read-only git status/ls-files/check-ignore; list repo top-level entries; verify the single approved app-code change · files: `reports/agents/batch-01-t-10.evidence.md` |
| 4b. Forbidden | git add/commit/push; deleting files to make status look clean; modifying .gitignore; touching data/ if it already exists; editing public/ |
| 5. Verification | app-diff-verify prints 'APP CODE DIFF OK'; the three data-safety empties print nothing; check-ignore prints the .gitignore rule that keeps data/ out of git; termux-batch list shows BATCH-01 with its task count. Any tracked file under data/ or uploads/ is a PII risk: report it, do not delete it. |
| 6. Expected report | Outputs of all six commands, plus a plain statement: 'member data cannot enter git from this working copy: yes/no', the single approved app-code change public/assets/js/app.js, and the list of files the batch itself produced. |
...
```bash
$ node scripts/app-diff-verify.mjs
$ git status --porcelain -- data uploads storage lib server.js agents
$ git check-ignore -v data/
$ git ls-files -- data uploads
$ git diff --name-only HEAD -- server.js lib agents storage
