...
**RE-CUT 2026-08-31:** the owner reported an intentional, non-destructive Priti AI change to
`public/assets/js/app.js` (143 added lines / 0 deleted) on the device. BATCH-01 was recut so it
preserves that change: `ui_change_approved: true`, the new fingerprint is captured on-device by
`termux-batch.mjs baseline BATCH-01` (never guessed by Arena), and T-01 regenerates the Guardian
design-lock from the preserved tree. D1 data is untouched.

| # | Item | Worker | Status | Blocks on | Why it is in the queue (evidence) |
| --- | --- | --- | --- | --- | --- |
| Q-01 | Run BATCH-01 on Termux; return the result batch | manager | **VERIFY** | device | Arena proved every local verifier green in its own sandbox before the Priti AI change (syntax 41/0, health 95/0, e2e 134/0 default **and** `PJS_STORAGE=json`, storage doctor PASS, worktree clean). BATCH-01 is recut for the preserved current UI: T-01 captures the approved baseline on-device, then T-02…T-10 prove the same checks against that preserved tree, plus the two things Arena structurally cannot see. |
