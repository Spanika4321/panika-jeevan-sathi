...
When an intentional UI change is approved the batch must also carry
`ui_change_reason` and `protected.public_ui_fingerprint_mode: "capture-on-device"`; the
device then establishes the new baseline with `node scripts/termux-batch.mjs baseline <BATCH>`
instead of Arena guessing a fingerprint it cannot see.
...
node scripts/termux-batch.mjs baseline BATCH-01   # ONLY when the batch approved an intentional UI change
#   (read-only against public/**; writes <BATCH>.baseline.json + regenerates reports/ui-baseline-body.md5
#    from the preserved tree. The runner refuses to `run` until this has been done.)
...
| Path | Role |
| --- | --- |
| `scripts/termux-batch.mjs` | runner + validator + renderer (`list/show/preflight/baseline/run/validate/render/template/decide`) |
| `ops/batches/BATCH-NN.tasks.json` | the batch Arena issues |
| `ops/batches/BATCH-NN.baseline.json` | the device-captured approved UI fingerprint (required for any `ui_change_approved: true` batch) |
