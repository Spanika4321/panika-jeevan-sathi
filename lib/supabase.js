...
/*
 * Tables an older installation may legitimately be missing. A missing optional
 * table degrades one feature (analytics / invite attribution); every member
 * table above is still required, so a broken member table never boots.
 */
const OPTIONAL_TABLES = ['site_stats', 'site_visitors', 'referrals'];
...
function createClient(config, options = {}) {
  ...
  async function ensureBucket() {
    try {
      const { json } = await send(`${storageBase}/bucket/${encodeURIComponent(bucket)}`, {
        method: 'GET',
        headers: authHeaders()
      });
      if (!json || json.public !== false)
        throw new SupabaseError('The photo storage bucket must be private (public=false). Refusing to expose member photos.', { status: 400 });
      return;
    } catch (err) {
    ...
    try {
      ...
      if (err.status === 409) {
        const { json } = await send(`${storageBase}/bucket/${encodeURIComponent(bucket)}`, { method: 'GET', headers: authHeaders() });
        if (!json || json.public !== false) throw new SupabaseError('The existing photo bucket must be private.', { status: 400 });
        return;
      }
      throw err;
    }
...
function createDriver(client, options = {}) {
  ...
  return {
    ...
    async load() {
      try {
        loaded = false;
        if (!await client.ping()) throw new Error('Invalid Supabase database response');
        ...
        for (const table of Object.keys(TABLES)) {
          try {
            ...
            // Analytics and invite attribution are optional for older
            // installations; all member tables are required.
            if (!OPTIONAL_TABLES.includes(table)) throw err;
            log(`[supabase] optional table ${table} is unavailable`);
          }
        }
        loaded = true;
        lastError = null;
