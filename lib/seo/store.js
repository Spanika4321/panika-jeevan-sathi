...
function createStore(options = {}) {
  ...
  /**
   * Rewrite a stored report with its final content.
   *
   * The first save happens before the last two cycle stages (report, verify)
   * exist, so the persisted copy would otherwise carry an incomplete trail.
   * Finalising keeps the stored report truthful about the whole cycle, and the
   * checksum is recomputed from the final payload.
   */
  async function finalizeReport(id, report, { s3 = null } = {}) {
    ...
    const checksum = sha256(payload);
    const markdown = toMarkdown(report);
    const patch = { payload, markdown, checksum };

    const result = { id: Number(id), checksum, archive: s3 ? 'PENDING' : 'NOT_CONFIGURED', error: '' };

    if (s3 && row.archive_key) {
      try {
        await s3.put(row.archive_key, `${JSON.stringify(Object.assign({ checksum }, report), null, 2)}\n`, 'application/json');
        await s3.put(row.archive_key.replace(/\.json$/, '.md'), markdown, 'text/markdown');
        await s3.put('reports/seo-report-latest.json', `${JSON.stringify(Object.assign({ checksum }, report), null, 2)}\n`, 'application/json');
        result.archive = 'SAVED';
      } catch (err) {
        result.archive = 'FAILED';
        result.error = String(err.message || err).slice(0, 300);
        patch.archive_status = 'FAILED';
        log(`[seo/store] Fil One re-archive failed: ${err.message}`);
      }
    }

    db.update('seo_reports', { id: Number(id) }, patch);
    return result;
  }
