...
function createSeoCenter(options = {}) {
  ...
  async function runCycle({ days = 28, site = '', trigger = 'manual' } = {}) {
    ...
    try {
      ...
      /* The first save happened before the last two stages existed. Rewrite the
         stored report with the complete trail and the verification result, then
         prove the final payload still matches its own checksum. */
      report.stages = stages.slice();
      report.storage_verification = storageVerification;
      const finalized = await store.finalizeReport(saved.id, report, { s3 });
      const reread = store.getReport(saved.id);
      const finalizedOk = Boolean(
        finalized && reread && reread.checksum === finalized.checksum && reread.report &&
          (reread.report.stages || []).length === stages.length
      );
      if (!finalizedOk) log('[seo] the stored report could not be finalised with the full stage trail');
