...
const doctorLib = require('./doctor');
...
function createApi(options) {
  ...
  route('GET', '/api/health', async (ctx) => {
    const cloud = doctorLib.envStatus();
    cloud.active = { database: db.kind || 'unknown', photos: photos.kind };
    if (ctx.query && ctx.query.get('doctor') === '1') {
      cloud.live = await doctorLib.liveCheck();
    }
    return {
      status: 200,
      body: {
