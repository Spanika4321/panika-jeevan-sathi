...
function sendJson(res, status, payload) {
  ...
  const acceptsGzip = Boolean(
    req && /(^|[,\s])gzip($|[,\s;])/i.test(String(req.headers['accept-encoding'] || ''))
  );
  if (acceptsGzip && Buffer.byteLength(body) >= 512) {
...
function createApi(options) {
  ...
  /**
   * Query a big IN-list in chunks. Keeps every SQL statement small and avoids
   * engine limits on the number of bound variables as membership grows to
   * 5k / 10k / 20k+ members.
   */
  function batchedIn(table, col, ids, extra = {}, opts = {}) {
    const values = [...new Set(ids.map(Number))];
    const out = [];
    for (let i = 0; i < values.length; i += 1000) {
      const chunk = values.slice(i, i + 1000);
      if (!chunk.length) continue;
      out.push(...db.all(table, Object.assign({ [col]: { in: chunk } }, extra), opts));
    }
    return out;
  }
  ...
  function decorate(profile, user) {
    const owner = user || userRow(profile.user_id);
  ...
  function visibleUsers() {
    ...
    // Batch: a few queries for all user records instead of one lookup per
    // profile (keeps search/matches fast as membership grows to 5k / 10k / 20k+).
    const users = batchedIn('users', 'id', rows.map((r) => r.user_id));
    const byId = new Map();
    for (const u of users) byId.set(Number(u.id), u);
    const out = [];
    for (const row of rows) {
      const user = byId.get(Number(row.user_id));
      if (!user || user.status !== 'active') continue;
      if (row.visibility === 'hidden') continue;
      const full = decorate(row, user);
  ...
  route('GET', '/api/profiles', async (ctx) => {
    ...
    const cardCtx = cardContext(matched, ctx.user.id);
    const decorated = matched.map((p) => {
      const { score, reasons } = profiles.matchScore(viewerProfile, p, { photo: p.photo });
      return { card: cardFor(p, ctx.user.id, cardCtx), score, reasons };
    });
  ...
  /**
   * Pre-compute, in a handful of batched queries, everything cardFor() needs
   * for a whole result set. Replaces the per-profile N+1 lookup pattern so
   * search/matches stay fast at 5k / 10k / 20k+ members.
   */
  function cardContext(profileList, viewerId) {
    const ids = [...new Set(profileList.map((p) => Number(p.user_id)))];
    const byUser = new Map(
      ids.map((id) => [id, { received: 0, interest: null, shortlisted: false, can_message: false }])
    );
    if (!ids.length) return byUser;

    for (const row of batchedIn('interests', 'from_user_id', ids, { status: 'accepted' })) {
      const e = byUser.get(Number(row.from_user_id));
      if (e) e.received += 1;
    }

    if (viewerId) {
      for (const row of batchedIn('interests', 'to_user_id', ids, { from_user_id: viewerId })) {
        const e = byUser.get(Number(row.to_user_id));
        if (e && e.interest === null) e.interest = row.status;
      }
      const shortlisted = new Set(
        batchedIn('shortlist', 'target_user_id', ids, { user_id: viewerId }).map((r) => Number(r.target_user_id))
      );
      const acceptedOut = new Set(
        batchedIn('interests', 'to_user_id', ids, { from_user_id: viewerId, status: 'accepted' }).map((r) => Number(r.to_user_id))
      );
      const acceptedIn = new Set(
        batchedIn('interests', 'from_user_id', ids, { to_user_id: viewerId, status: 'accepted' }).map((r) => Number(r.from_user_id))
      );
      for (const id of ids) {
        const e = byUser.get(id);
        if (shortlisted.has(id)) e.shortlisted = true;
        e.can_message = acceptedOut.has(id) || acceptedIn.has(id);
      }
    }
    return byUser;
  }

  function cardFor(p, viewerId, ctx) {
    const received = ctx
      ? ctx.get(Number(p.user_id)).received
      : db.count('interests', { from_user_id: p.user_id, status: 'accepted' });
    let interest = null;
    let shortlisted = false;
    let can_message = false;
    if (viewerId) {
      if (ctx) {
        const e = ctx.get(Number(p.user_id));
        interest = e.interest;
        shortlisted = e.shortlisted;
        can_message = e.can_message;
      } else {
  ...
  route('GET', '/api/matches', async (ctx) => {
    ...
    const cardCtx = cardContext(candidates, ctx.user.id);
    const receivedFrom = new Set(
      batchedIn('interests', 'from_user_id', candidates.map((p) => p.user_id), {
        to_user_id: ctx.user.id,
        status: { in: ['pending', 'accepted'] }
      }).map((r) => Number(r.from_user_id))
    );
    const scored = candidates.map((p) => {
      const { score, reasons } = profiles.matchScore(viewerProfile, p, {
        photo: p.photo,
        receivedInterest: receivedFrom.has(Number(p.user_id))
      });
      return { card: cardFor(p, ctx.user.id, cardCtx), score, reasons };
    });
  ...
  route('GET', '/api/shortlist', async (ctx) => {
    ...
    if (!rows.length) return ok(ctx.res, { results: [] });
    const ids = [...new Set(rows.map((r) => Number(r.target_user_id)))];
    const users = batchedIn('users', 'id', ids);
    const profiles = batchedIn('profiles', 'user_id', ids);
    const userById = new Map(users.map((u) => [Number(u.id), u]));
    const profileById = new Map(profiles.map((p) => [Number(p.user_id), p]));
    const cardCtx = cardContext(ids.map((id) => ({ user_id: id })), ctx.user.id);
    const out = rows
      .map((r) => {
        const other = userById.get(Number(r.target_user_id));
        const otherProfile = profileById.get(Number(r.target_user_id));
        if (!other || !otherProfile) return null;
        return Object.assign(
          cardFor(Object.assign({}, otherProfile, { name: other.name, photo: other.photo }), ctx.user.id, cardCtx),
          {
