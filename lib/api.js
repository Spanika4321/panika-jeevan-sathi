...
function createApi(options) {
  ...
  // Photo storage: local disk, R2, or the D1 bridge used while an R2 bucket is
  // unavailable. Remote stores keep only a disposable cache in this folder.
  ...
  function savePhoto(userId, dataUrl) {
    ...
    const storageLimit = Math.min(MAX_PHOTO_BYTES, Number(photos.maxBytes || MAX_PHOTO_BYTES));
    if (buffer.length > storageLimit) {
      const label = storageLimit >= 1024 * 1024
        ? `${Math.floor(storageLimit / (1024 * 1024))} MB`
        : `${Math.floor(storageLimit / 1024)} KB`;
      throw Object.assign(new Error(`Image is too large (max ${label}).`), { status: 413 });
    }
