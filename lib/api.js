...
function createApi(options) {
  ...
  route('GET', '/api/admin/backup', async (ctx) => {
    if (!requireAdmin(ctx)) return;
    return ok(ctx.res, backupLib.status({ dataDir }));
  });

  route('POST', '/api/admin/backup', async (ctx) => {
    if (!requireAdmin(ctx)) return;
    try {
      const result = await backupLib.create({ dataDir, driver: db, label: 'admin', tag: 'admin', verify: true });
      audit(ctx.user, 'admin.backup', {
        detail: `${result.file} · ${Math.round(result.bytes / 1024)} KB · ${
          result.counts && result.counts.users !== undefined ? result.counts.users : '?'
        } members`
      });
      return ok(ctx.res, {
        file: result.file,
        ...
        status: backupLib.status({ dataDir })
      });
    } catch (err) {
      console.error('[backup]', err);
      return fail(ctx.res, 500, 'The backup could not be written. Check the server log for the reason.');
    }
  });

  route('GET', '/api/admin/backup/download', async (ctx) => {
    if (!requireAdmin(ctx)) return;
    const requested = path.basename(str(ctx.query.get('file')));
    if (!backupLib.NAME_RE.test(requested)) return fail(ctx.res, 400, 'That is not a backup file name.');
    const dir = backupLib.backupDirFor(dataDir);
    const file = path.join(dir, requested);
    let stat;
    try {
      stat = fs.statSync(file);
    } catch (_) {
      return fail(ctx.res, 404, 'That backup is no longer on the server.');
    }
    audit(ctx.user, 'admin.backup.download', { detail: requested });
    // Written straight to the socket: this is a binary download, not JSON.
  ...
  route('GET', '/api/admin/backup/verify', async (ctx) => {
    if (!requireAdmin(ctx)) return;
    const requested = path.basename(str(ctx.query.get('file')));
    if (!backupLib.NAME_RE.test(requested)) return fail(ctx.res, 400, 'That is not a backup file name.');
    const file = path.join(backupLib.backupDirFor(dataDir), requested);
    if (!fs.existsSync(file)) return fail(ctx.res, 404, 'That backup is no longer on the server.');
    const result = backupLib.verifyArchive(file);
    return ok(ctx.res, { file: requested, verified: result.ok, detail: result.detail });
  });
