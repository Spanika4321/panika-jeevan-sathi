...
function createApi(options) {
  ...
  route('POST', '/api/auth/register', async (ctx) => {
    ...
    // Fire-and-forget backup to Google Sheet (never blocks registration).
    try {
      const freshUser = db.one('users', { id: user.id });
      const freshProfile = db.one('profiles', { user_id: user.id });
      gsheet.pushRegistration(freshUser, freshProfile);
    } catch (_gsheetErr) {
      /* Sheet backup failure must never affect the registration response. */
    }
