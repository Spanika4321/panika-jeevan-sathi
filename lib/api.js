  ...
  return new Promise((resolve, reject) => {
    ...
    req.on('data', (chunk) => {
      ...
      if (size > MAX_BODY_BYTES) {
        ...
        // Drain, rather than destroy, the request so the client receives the 413.
        return;
      }
      chunks.push(chunk);
    });
    req.on('end', () => {
      ...
      try {
        ...
        return reject(Object.assign(new Error('Invalid JSON body'), { status: 400 }));
      }
      if (!body || typeof body !== 'object' || Array.isArray(body)) {
        return reject(Object.assign(new Error('JSON body must be an object'), { status: 400 }));
      }
      resolve(body);
    });
...
function baseUrl(req) {
  // Pin emailed tokens to the deployment origin; never trust a forwarded host
  // over SITE_URL (otherwise a forged host can steal password-reset tokens).
...
function createApi(options) {
  ...
  function throttled(key, limit, windowMs) {
    ...
    if (now >= nextRateLimitCleanup) {
      for (const [oldKey, value] of rateLimit) if (value.reset <= now) rateLimit.delete(oldKey);
      nextRateLimitCleanup = now + 60000;
    }
  ...
  /**
   * Record who invited a brand-new member, then tell that member.
   *
   * Rules (all enforced here, never trusted from the client):
   *  - the code must decode to a real, ACTIVE member (a suspended or deleted
   *    inviter gets no credit);
   *  - self-referral is ignored;
   *  - a member can only be attributed once, ever;
   *  - nothing about the new member is revealed to the inviter;
   *  - any storage failure returns null instead of breaking sign-up.
   */
  async function attributeReferral(newUser, code) {
    try {
      const inviterId = referral.userIdFromCode(code);
      if (!inviterId || inviterId === Number(newUser.id)) return null;
      const inviter = await db.one('users', { id: inviterId });
      if (!inviter || inviter.status !== 'active') return null;
      const recorded = await referral.record(db, {
        inviterId,
        inviteeId: newUser.id,
        code: String(code || '').toUpperCase().slice(0, 16),
      ...
      await notify(
        inviterId,
        'referral',
        'A new member joined with your invite link',
        'Your invite link is working. Share it with more families from our community — it is completely free.',
        '/dashboard.html#invite'
      );
      await audit(inviter, 'referral.joined', {
        target_type: 'user',
        target_id: Number(newUser.id),
        detail: 'new member registered with invite code'
      });
      return inviterId;
    } catch (err) {
  ...
  async function visibleUsers() {
    ...
    for (const row of rows) {
      ...
      if (!user || user.status !== 'active' || Number(user.email_verified) !== 1) continue;
  ...
  function visibleTo(user, profile, viewer) {
    if (!user || !profile) return false;
    if (viewer && (viewer.id === user.id || viewer.role === 'admin')) return true;
    return user.status === 'active' && Number(user.email_verified) === 1 &&
      profile.visibility !== 'hidden' && (profile.visibility === 'everyone' || Boolean(viewer));
  }

  function relationshipUser(user, profile, viewer) {
    if (!visibleTo(user, profile, viewer)) {
      return { id: user.id, name: 'Unavailable member', photo: null, city: '', age: null, available: false };
    }
    return {
      id: user.id, name: user.name,
      photo: Number(profile.hide_photo) === 1 ? null : user.photo || null,
      age: profile.age, city: profile.city, state: profile.state, community: profile.community,
      education: profile.education, occupation: profile.occupation, available: true
    };
  }
  ...
  async function promoteOwner(user) {
    if (!user || !owner.isOwnerEmail(user.email) || user.status !== 'active' || Number(user.email_verified) !== 1) return user;
    if (user.role === 'admin') return user;
    await db.update('users', { id: user.id }, { role: 'admin', token_version: Number(user.token_version || 1) + 1, reset_token: null, reset_expires: 0 });
  ...
  async function currentUser(req) {
    ...
    const user = await userRow(payload.uid);
    ...
    const effective = await promoteOwner(user);
    return Number(effective.token_version || 1) === Number(payload.tv) ? effective : null;
  }

  function issueSession(res, user, req) {
    const token = auth.createSession(user.id, Number(user.token_version || 1), secret);
    const secure = httpSecurity.isSecure(req);
    res.setHeader('Set-Cookie', auth.sessionCookie(token, { secure }));
  }

  /* ---------------------------------------------------------------- photos */

  async function canViewPhoto(req, pathname) {
    if (!/^\/uploads\/[A-Za-z0-9._-]+$/.test(pathname)) return false;
    const user = await db.one('users', { photo: pathname });
    if (!user) return false;
    const viewer = await currentUser(req);
    if (viewer && (viewer.id === user.id || viewer.role === 'admin')) return true;
    if (user.status !== 'active' || Number(user.email_verified) !== 1) return false;
    const profile = await profileRow(user.id);
    if (!profile || profile.visibility === 'hidden' || Number(profile.hide_photo) === 1) return false;
    return profile.visibility === 'everyone' || Boolean(viewer);
  }
  ...
  const routes = [];
  let adminMutations = Promise.resolve();
  function route(method, pattern, handler) {
    const parts = pattern.split('/').filter(Boolean);
    const perform = handler;
    if (pattern.startsWith('/api/admin/') && ['POST', 'PUT', 'PATCH', 'DELETE'].includes(method)) {
      handler = async (ctx) => {
        const previous = adminMutations;
        let release;
        adminMutations = new Promise((resolve) => { release = resolve; });
        await previous;
        try {
          // Avoid concurrent last-admin changes and recheck access after waiting.
          ctx.user = await currentUser(ctx.req);
          return await perform(ctx);
        } finally { release(); }
      };
    }
    routes.push({
  ...
  route('GET', '/api/health', async () => {
    ...
    const remote = options.remoteStatus ? options.remoteStatus() : null;
    const remoteConfigured = ['supabase', 'd1'].includes(storage) && Boolean(photos.remote);
    const healthy = !remote?.database?.lastError && !remote?.photos?.lastError && remote?.database?.loaded !== false;
    const durable = remoteConfigured && healthy && !(remote?.database?.pending > 0) && !(remote?.photos?.pending > 0);
    // Provider error strings may include SQL values or object paths. Detailed
    // diagnostics stay in authenticated admin storage tools, not public health.
    const publicRemote = remote ? {
      database: { ...remote.database, url: undefined, lastError: remote.database?.lastError ? 'Database operation failed' : null },
      photos: { ...remote.photos, lastError: remote.photos?.lastError ? 'Photo storage operation failed' : null }
    } : null;
    return {
      status: healthy ? 200 : 503,
      body: {
        ok: healthy, service: 'panika-jeevan-sathi', time: Date.now(), boot_at: BOOT_AT,
        security_revision: '2026-09-05',
        release: process.env.RENDER_GIT_COMMIT || null,
        storage, photos: photos.kind, durable, data_loss_risk: !durable,
  ...
  route('POST', '/api/auth/register', async (ctx) => {
    ...
    const requireVerification = owner.isOwnerEmail(email) || await settingsLib.get(db, 'require_email_verification') === '1';
    const now = Date.now();
    const user = await db.insert('users', {
      ...
      verification_token: requireVerification ? auth.randomToken(24) : null,
      reset_token: null,
      reset_expires: 0,
      token_version: 1,
      photo: null,
    ...
    /* Growth loop: attribute this member to whoever shared their invite link.
       Deliberately best-effort - a bad, stale or forged code, a missing
       `referrals` table or a storage hiccup must never fail a registration. */
    const inviterId = await attributeReferral(user, str(body.ref));
    ...
    if (requireVerification) ctx.res.setHeader('Set-Cookie', auth.clearSessionCookie());
    else issueSession(ctx.res, await db.one('users', { id: user.id }), ctx.req);
    return ok(ctx.res, {
  ...
  route('POST', '/api/auth/login', async (ctx) => {
    ...
    const email = str(body.email).toLowerCase();
    const password = passwordValue(body.password);
    if (!isEmail(email) || !password || password.length > 1024)
  ...
  route('POST', '/api/auth/resend-verification', async (ctx) => {
    ...
    if (!isEmail(email)) return fail(ctx.res, 400, 'Please enter a valid email address.');
    if (throttled(`verify-ip:${ctx.ip}`, 20, 10 * 60000) || throttled(`verify:${email}`, 3, 10 * 60000))
      return fail(ctx.res, 429, 'Too many verification requests. Please try again later.');
    const user = await db.one('users', { email });
    if (user && user.status === 'active' && Number(user.email_verified) !== 1) {
      const token = user.verification_token || auth.randomToken(24);
      await db.update('users', { id: user.id }, { verification_token: token });
      await sendVerificationMail(user, token, ctx.req);
    }
    return ok(ctx.res, {
      message: 'If this account needs verification, check your email for a link. If it does not arrive, contact support.'
    });
  });
  ...
  route('POST', '/api/auth/forgot', async (ctx) => {
    const email = str(ctx.body.email).toLowerCase();
    if (!isEmail(email)) return fail(ctx.res, 400, 'Please enter a valid email address.');
    if (throttled(`forgot-ip:${ctx.ip}`, 30, 15 * 60000) || throttled(`forgot:${email}`, 5, 15 * 60000))
      return fail(ctx.res, 429, 'Too many reset requests. Please try again later.');
    const user = await db.one('users', { email });
    if (user) {
      const token = auth.randomToken(24);
      await db.update('users', { id: user.id }, { reset_token: token, reset_expires: Date.now() + 3600000 });
      await sendResetMail(user, token, ctx.req);
    }
    // Identical response for known and unknown emails. Tokens belong only in
    // mail/the private admin outbox, never in an unauthenticated API response.
    return ok(ctx.res, {
      message: 'If that email is registered, check your inbox for a password reset link. If it does not arrive, contact support.'
    });
  });

  route('POST', '/api/auth/reset', async (ctx) => {
    const token = str(ctx.body.token);
    if (!/^[a-f0-9]{48}$/.test(token)) return fail(ctx.res, 400, 'This reset link is invalid or has expired.');
    const password = passwordValue(ctx.body.password);
    ...
    const changed = await db.update(
      'users',
      { id: user.id, reset_token: token, reset_expires: { gte: Date.now() } },
      {
        password_hash: auth.hashPassword(password),
        reset_token: null,
        reset_expires: 0,
    ...
    if (!changed) return fail(ctx.res, 400, 'This reset link is invalid or has expired.');
  ...
  route('GET', '/api/me', async (ctx) => {
    ...
    return ok(ctx.res, {
      ...
      /* The member's own invite link (growth loop). The code is derived from
         the member id, so it is stable and needs no storage. `invited` is null
         when the optional referrals table is not present on this install. */
      invite: {
        code: referral.codeFor(user.id),
        path: referral.invitePath(user.id),
        invited: await referral.countFor(db, user.id)
      }
    });
  });
  ...
  route('POST', '/api/me/password', async (ctx) => {
    if (!ctx.user) return fail(ctx.res, 401, 'Please log in to continue.');
    const current = passwordValue(ctx.body.current_password);
    const next = passwordValue(ctx.body.new_password);
  ...
  route('PUT', '/api/profile', async (ctx) => {
    ...
    const { data, errors } = profiles.validatePatch(ctx.body, existing);
  ...
  route('GET', '/api/profiles/:id', async (ctx) => {
    ...
    if (!owner || owner.status !== 'active' || (Number(owner.email_verified) !== 1 && !(ctx.user && ctx.user.role === 'admin')))
  ...
  route('GET', '/api/profiles', async (ctx) => {
    ...
    const matched = candidates.filter((p) => filter(p, { name: p.name, photo: Number(p.hide_photo) === 1 ? null : p.photo }));
    ...
    const page = positiveInteger(ctx.query.get('page'), 1);
    const perPage = positiveInteger(ctx.query.get('per_page'), 12, 60);
  ...
  route('GET', '/api/matches', async (ctx) => {
    ...
    const limit = positiveInteger(ctx.query.get('limit'), 12, 60);
    const candidates = (await visibleUsers()).filter(
      (p) => Number(p.searchable) === 1 && p.user_id !== ctx.user.id
    );
    const scored = [];
  ...
  route('POST', '/api/interests', async (ctx) => {
    ...
    if (!visibleTo(target, targetProfile, ctx.user))
      return fail(ctx.res, 404, 'This profile is not available.');
  ...
  route('GET', '/api/interests', async (ctx) => {
    ...
    for (const r of rows) {
      ...
      out.push({
        ...
        direction,
        user: relationshipUser(other, otherProfile, ctx.user)
      });
    }
  ...
  route('POST', '/api/shortlist', async (ctx) => {
    ...
    if (!visibleTo(await userRow(targetId), await profileRow(targetId), ctx.user))
      return fail(ctx.res, 404, 'Profile not found.');
  ...
  route('GET', '/api/shortlist', async (ctx) => {
    ...
    for (const r of rows) {
      ...
      if (!visibleTo(other, otherProfile, ctx.user)) continue;
      out.push(
        Object.assign(
  ...
  route('GET', '/api/conversations/:id', async (ctx) => {
    ...
    const other = await userRow(otherId);
    const otherProfile = await profileRow(otherId);
    if (!other || !otherProfile || (!visibleTo(other, otherProfile, ctx.user) && !rows.length))
      return fail(ctx.res, 404, 'This conversation is not available.');
    return ok(ctx.res, {
      with: relationshipUser(other, otherProfile, ctx.user),
      connected: visibleTo(other, otherProfile, ctx.user) && await acceptedPair(ctx.user.id, otherId),
      messages: rows.map((m) => ({
        id: m.id,
        body: m.body,
  ...
  route('POST', '/api/messages', async (ctx) => {
    ...
    if (!target || target.status !== 'active' || !visibleTo(target, await profileRow(to), ctx.user))
      return fail(ctx.res, 404, 'Recipient not found.');
  ...
  async function activeAdmins() {
    return (await db.all('users')).filter((u) => u.role === 'admin' && u.status === 'active' && Number(u.email_verified) === 1);
  }
  ...
  route('GET', '/api/admin/stats', async (ctx) => {
    ...
    return ok(ctx.res, {
      ...
      recent_users,
      recent_logins,
      /* Invite-link rollup: which members are actually bringing new families.
         null when the optional `referrals` table is not available. */
      invites: await referral.summary(db, { limit: 8 })
    });
  });
  ...
  route('PATCH', '/api/admin/users/:id', async (ctx) => {
    ...
    if (ctx.body.email_verified !== undefined) {
      patch.email_verified = [true, 1, '1'].includes(ctx.body.email_verified) ? 1 : 0;
      if (!patch.email_verified && user.role === 'admin' && (await activeAdmins()).filter((u) => u.id !== user.id).length < 1)
        return fail(ctx.res, 400, 'Cannot unverify the last administrator.');
    }
    if (['role', 'status', 'email_verified'].some((key) => patch[key] !== undefined && patch[key] !== user[key])) {
      patch.token_version = Number(user.token_version || 1) + 1;
      patch.reset_token = null;
      patch.reset_expires = 0;
    }
    if (ctx.body.password) {
      const problem = auth.passwordProblem(ctx.body.password);
      if (problem) return fail(ctx.res, 400, problem);
      patch.password_hash = auth.hashPassword(passwordValue(ctx.body.password));
      patch.reset_token = null;
      patch.reset_expires = 0;
      patch.token_version = Number(user.token_version || 1) + 1;
    }
  ...
  route('PATCH', '/api/admin/reports/:id', async (ctx) => {
    ...
    const target = await userRow(report.target_user_id);
    if (target && ['suspend', 'delete'].includes(ctx.body.action)) {
      if (target.id === ctx.user.id || (target.role === 'admin' && (await activeAdmins()).filter((u) => u.id !== target.id).length < 1))
        return fail(ctx.res, 400, 'Cannot remove your own access or the last administrator.');
    }
    await db.update('reports', { id }, { status });
    if (ctx.body.action === 'suspend' && target) {
      await db.update('users', { id: target.id }, { status: 'suspended', token_version: Number(target.token_version || 1) + 1, reset_token: null, reset_expires: 0 });
      await notify(
  ...
  route('GET', '/api/admin/audit', async (ctx) => {
    if (!requireAdmin(ctx)) return;
    const limit = positiveInteger(ctx.query.get('limit'), 80, 200);
  ...
  async function handle(req, res, url) {
    const ctx = {
      ...
      params: {},
      body: {},
      ip: httpSecurity.clientIp(req),
      user: null
    };
    try {
      const problem = httpSecurity.writeRequestProblem(req);
      if (problem) {
        req.resume();
        return fail(res, problem.status, problem.error);
      }
      ...
      const isAuthRoute = url.pathname.startsWith('/api/auth') || url.pathname === '/api/site' || url.pathname === '/api/health';
  ...
  return { handle, canViewPhoto, uploadDir, outboxDir };
}
