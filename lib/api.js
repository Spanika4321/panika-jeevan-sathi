...
function createApi(options) {
  ...
  const uploadDir = path.join(dataDir, UPLOAD_DIR_NAME);
  ...
  function deletePhotoFile(photoPath) {
    if (!photoPath) return;
    const name = path.basename(String(photoPath));
    if (!name) return;
    try {
  ...
  function savePhoto(userId, dataUrl) {
    ...
    db.update('users', { id: userId }, { photo: `/uploads/${name}` });
    return `/uploads/${name}`;
  }
  ...
  route('GET', '/api/health', async () => ({ status: 200, body: { ok: true, service: 'panika-jeevan-sathi', time: Date.now() } }));
  ...
  route('POST', '/api/support/payment', async (ctx) => {
    const body = ctx.body || {};
    const name = str(body.name) || (ctx.user && ctx.user.name) || '';
    const email = str(body.email).toLowerCase() || (ctx.user && ctx.user.email) || '';
    const kind = str(body.kind).toLowerCase() || 'support';
    if (!['support', 'featured', 'ad'].includes(kind))
      return fail(ctx.res, 400, 'Please choose a valid support type.');
    if (name.length < 2) return fail(ctx.res, 400, 'Please enter your name.');
    if (email && !isEmail(email)) return fail(ctx.res, 400, 'Please enter a valid email address.');
    const amount = earn.parseAmount(body.amount);
    if (amount === null) return fail(ctx.res, 400, 'Please enter a valid amount.');
    const upiRef = str(body.upi_ref).replace(/\s+/g, '').slice(0, 40);
    if (upiRef.length < 4) return fail(ctx.res, 400, 'Please enter the UPI reference / UTR number from your payment app.');
    if (throttled(`pay:${ctx.ip}`, 8, 60 * 60000))
      return fail(ctx.res, 429, 'Too many payment notes. Please try again later.');
    const row = db.insert('payments', {
      user_id: ctx.user ? ctx.user.id : 0,
    ...
    if (ctx.user) {
      notify(
        ...
        'system',
        'UPI note received',
        'Thank you. Our team will confirm your UPI transfer and write back on WhatsApp if needed.',
        '/support.html'
      );
    }
    return ok(ctx.res, {
      id: row.id,
      message: 'Thank you. Your UPI note is with our team. Member features stay free — this is optional support only.'
    });
  });
  ...
  route('GET', '/api/profiles/:id', async (ctx) => {
    ...
    const payload = {
      ...
      featured: earn.isFeatured(profile),
      interest_state: null,
      shortlisted: false,
      can_message: false
    };
  ...
  route('GET', '/api/profiles', async (ctx) => {
    ...
    decorated.sort((a, b) => {
      const fa = a.card.featured ? 1 : 0;
      const fb = b.card.featured ? 1 : 0;
  ...
  route('GET', '/api/admin/stats', async (ctx) => {
    ...
    return ok(ctx.res, {
      stats: {
        ...
        payments_pending: db.count('payments', { status: 'pending' }),
        featured: db.all('profiles').filter((p) => earn.isFeatured(p)).length,
      ...
      return Object.assign(serializeUser(u, { private: true }), {
        profile: p
          ? {
              ...
              featured: earn.isFeatured(p),
              featured_until: p.featured_until || 0
            }
          : null
      });
    });
  ...
  route('POST', '/api/admin/users/:id/feature', async (ctx) => {
    if (!requireAdmin(ctx)) return;
    const id = Number(ctx.params.id);
    if (!userRow(id) || !profileRow(id)) return fail(ctx.res, 404, 'User not found.');
    const daysRaw = ctx.body && ctx.body.days !== undefined ? Number(ctx.body.days) : Number(settingsLib.get(db, 'featured_days') || 30);
    const days = Number.isFinite(daysRaw) && daysRaw >= 1 && daysRaw <= 365 ? Math.round(daysRaw) : 30;
    const until = Date.now() + days * 86400000;
    db.update('profiles', { user_id: id }, { featured_until: until, updated_at: Date.now() });
    notify(
      id,
      'system',
      'Your profile is featured',
      `Your profile will appear at the top of search and matches for ${days} days. Thank you for supporting the community site.`,
      '/dashboard.html'
    );
    audit(ctx.user, 'profile.feature', {
      target_type: 'user',
      target_id: id,
      detail: `${days} days`
    });
    return ok(ctx.res, {
      ...
      message: `Profile featured for ${days} days.`
    });
  });

  route('DELETE', '/api/admin/users/:id/feature', async (ctx) => {
    if (!requireAdmin(ctx)) return;
    const id = Number(ctx.params.id);
    if (!userRow(id) || !profileRow(id)) return fail(ctx.res, 404, 'User not found.');
    db.update('profiles', { user_id: id }, { featured_until: 0, updated_at: Date.now() });
    audit(ctx.user, 'profile.unfeature', { target_type: 'user', target_id: id, detail: '' });
    return ok(ctx.res, { message: 'Featured listing removed.' });
  });
  ...
  route('GET', '/api/admin/payments', async (ctx) => {
    if (!requireAdmin(ctx)) return;
    const status = String(ctx.query.get('status') || '');
    const rows = db.all('payments', {}, { order: '-created_at', limit: 200 });
    const out = rows
      .filter((r) => !status || r.status === status)
      .map((r) => {
        const member = r.user_id ? userRow(r.user_id) : null;
        const profile = r.user_id ? profileRow(r.user_id) : null;
        return Object.assign({}, r, {
          member: member ? { id: member.id, name: member.name, email: member.email } : null,
          featured: profile ? earn.isFeatured(profile) : false
        });
      });
    return ok(ctx.res, { payments: out });
  });

  route('PATCH', '/api/admin/payments/:id', async (ctx) => {
    if (!requireAdmin(ctx)) return;
    const id = Number(ctx.params.id);
    const row = db.one('payments', { id });
    if (!row) return fail(ctx.res, 404, 'Payment note not found.');
    const status = str(ctx.body.status);
    if (!['pending', 'confirmed', 'rejected'].includes(status))
      return fail(ctx.res, 400, 'Invalid status.');
    db.update('payments', { id }, { status, reviewed_at: Date.now() });
    if (status === 'confirmed' && row.kind === 'featured' && row.user_id) {
      const daysRaw = Number(settingsLib.get(db, 'featured_days') || 30);
      const days = Number.isFinite(daysRaw) && daysRaw >= 1 ? Math.round(daysRaw) : 30;
      db.update('profiles', { user_id: row.user_id }, { featured_until: Date.now() + days * 86400000, updated_at: Date.now() });
      notify(
        row.user_id,
        'system',
        'Your profile is featured',
        `UPI support confirmed. Your profile is now featured for ${days} days.`,
        '/dashboard.html'
      );
    }
    if (row.user_id && status === 'rejected') {
      notify(
        row.user_id,
        'system',
        'UPI note not confirmed',
        'We could not match this UPI reference. Please WhatsApp us with the screenshot.',
        '/support.html'
      );
    }
    audit(ctx.user, 'payment.update', {
      target_type: 'payment',
      target_id: id,
      detail: `${status}${row.kind ? ' · ' + row.kind : ''}`
    });
    return ok(ctx.res, { message: 'Payment note updated.' });
  });
