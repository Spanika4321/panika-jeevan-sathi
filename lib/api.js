...
const appsScriptLib = require('./appsscript');
...
function createApi(options) {
  ...
  const sheetsBridge = options.sheets || {};

  /** Current link status + a live ping of the web app. */
  route('GET', '/api/admin/apps-script', async (ctx) => {
    if (!requireAdmin(ctx)) return;
    const config = sheetsBridge.config || null;
    const info = appsScriptLib.describe(config);
    let live = null;
    if (config && sheetsBridge.test) {
      try {
        live = { ok: true, ...(await sheetsBridge.test(config)) };
      } catch (err) {
    ...
    return ok(ctx.res, {
      ...
      live,
      storage: db.kind,
      storageStats: driverStats(),
      appScriptFile: appsScriptLib.CONFIG_FILE,
      note:
        'Saving a URL writes data/apps-script.json. Switch the site to the Sheet with ' +
        'PJS_STORAGE=sheets (Sheet is the database) or PJS_STORAGE=mirror (local database + ' +
        'a live copy in the Sheet), then restart the service.'
    });
  });

  /** Test a URL (body.test = true) or test and save it. */
  route('POST', '/api/admin/apps-script', async (ctx) => {
    if (!requireAdmin(ctx)) return;
    if (!sheetsBridge.test) return fail(ctx.res, 400, 'The Apps Script bridge is not available on this server.');
    const url = String(ctx.body.url || '').trim();
    const token = String(ctx.body.token || '').trim();
    const mode = String(ctx.body.mode || '').trim().toLowerCase();
    if (!url) return fail(ctx.res, 400, 'Paste the Apps Script web app URL (it ends with /exec).');
    if (!/^https:\/\/script\.google\.com\/macros\/s\/[A-Za-z0-9_-]+\/exec$/.test(url)) {
      return fail(
        ctx.res,
        400,
        'That does not look like a deployed web app URL. It must look like ' +
          'https://script.google.com/macros/s/AKfycb…/exec'
      );
    }
    const candidate = { url, token, mode, timeoutMs: appsScriptLib.DEFAULTS.timeoutMs };
    let ping = null;
    try {
      ping = await sheetsBridge.test(candidate);
    } catch (err) {
      return fail(ctx.res, 502, `The web app did not answer: ${err.message}`, { step: 'ping' });
    }
    if (ctx.body.test) return ok(ctx.res, { saved: false, tested: true, ping });

    if (!sheetsBridge.save) return fail(ctx.res, 400, 'This server cannot store the configuration.');
    sheetsBridge.save({ url, token, mode });
    return ok(ctx.res, {
      saved: true,
      tested: true,
      ping,
      ...
      note:
        'Saved. Restart the service with PJS_STORAGE=sheets (Sheet is the database) or ' +
        'PJS_STORAGE=mirror (keep the current database and copy changes into the Sheet).'
    });
  });

  /** Forget the saved URL. */
  route('DELETE', '/api/admin/apps-script', async (ctx) => {
    if (!requireAdmin(ctx)) return;
    if (sheetsBridge.clear) sheetsBridge.clear();
    return ok(ctx.res, { cleared: true, restartRequired: true });
  });

  /** Re-read the Sheet into memory (Google Sheets as the database). */
  route('POST', '/api/admin/apps-script/reload', async (ctx) => {
    if (!requireAdmin(ctx)) return;
    if (!sheetsBridge.reload) return fail(ctx.res, 400, 'Reload is not available here.');
    const info = await sheetsBridge.reload();
    if (!info) {
      return fail(
        ctx.res,
        400,
        'This site is not running with the Sheet as its database. Set PJS_STORAGE=sheets and restart.'
      );
    }
    return ok(ctx.res, { reloaded: true, rows: info.rows, tables: info.tables });
  });
  ...
  function driverStats() {
    try {
      return typeof db.stats === 'function' ? db.stats() : { kind: driverKind(db) };
    } catch (_) {
      return { kind: driverKind(db) };
    }
  }
