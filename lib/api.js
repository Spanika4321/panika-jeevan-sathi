...
function createApi(options) {
  ...
  // The SEO Center is optional at the API layer: lib/seo is injected by
  // server.js. Without it the rest of the site behaves exactly as before.
  const seo = options.seo;

  if (seo) {
    const SEO_THROTTLE_KEY = (ctx) => `seo-cycle:${(ctx.user && ctx.user.id) || ctx.ip}`;

    route('GET', '/api/seo/status', async (ctx) => {
      if (!requireAdmin(ctx)) return;
      return ok(ctx.res, await seo.status());
    });

    /** Start the Google OAuth flow (credentials never leave the server). */
    route('GET', '/api/seo/connect/start', async (ctx) => {
      if (!requireAdmin(ctx)) return;
      const result = seo.connectStart({ requestOrigin: baseUrl(ctx.req) });
      if (!result.ok) return fail(ctx.res, 400, result.reason, { state: result.state });
      audit(ctx.user, 'seo.connect.start', { target_type: 'seo', detail: 'Google OAuth flow started' });
      return ok(ctx.res, result);
    });

    /** Google redirects the browser here; the browser is then sent back to /seo.html. */
    route('GET', '/api/seo/connect/callback', async (ctx) => {
      const code = ctx.query.get('code') || '';
      const state = ctx.query.get('state') || '';
      const error = ctx.query.get('error') || '';
      const back = (target, message) => {
        const url = `/seo.html?${target}=${encodeURIComponent(message)}`;
      ...
      if (error) return back('connect_error', `Google returned an error: ${error}`);

      const result = await seo.connectCallback({ code, state });
      if (!result.ok) {
        audit(ctx.user, 'seo.connect.failed', { target_type: 'seo', detail: result.reason.slice(0, 300) });
        return back('connect_error', result.reason);
      }
      audit(ctx.user, 'seo.connect.ok', {
        target_type: 'seo',
        detail: `connected ${result.account || ''} ${result.site_url || ''}`.trim()
      });
      return back('connected', `Connected${result.site_url ? ` to ${result.site_url}` : ''}`);
    });

    route('POST', '/api/seo/disconnect', async (ctx) => {
      if (!requireAdmin(ctx)) return;
      const result = seo.disconnect();
      audit(ctx.user, 'seo.disconnect', { target_type: 'seo', detail: 'Google connection removed' });
      return ok(ctx.res, result);
    });

    route('GET', '/api/seo/properties', async (ctx) => {
      if (!requireAdmin(ctx)) return;
      const result = await seo.properties();
      if (!result.ok) return fail(ctx.res, 502, result.reason, { state: result.state });
      return ok(ctx.res, result);
    });

    route('GET', '/api/seo/overview', async (ctx) => {
      if (!requireAdmin(ctx)) return;
      const result = await seo.overview({
        site: ctx.query.get('site') || '',
        days: ctx.query.get('days') || 28
      });
      if (!result.ok) return fail(ctx.res, 502, result.reason, { state: result.state });
      // When nothing is connected this deliberately carries no numbers.
      return ok(ctx.res, result);
    });

    route('GET', '/api/seo/queries', async (ctx) => {
      if (!requireAdmin(ctx)) return;
      const result = await seo.queries({
        site: ctx.query.get('site') || '',
        days: ctx.query.get('days') || 28,
    ...
    route('GET', '/api/seo/pages', async (ctx) => {
      if (!requireAdmin(ctx)) return;
      const result = await seo.pages({
        site: ctx.query.get('site') || '',
        days: ctx.query.get('days') || 28,
        limit: ctx.query.get('limit') || 100
      });
    ...
    /** Run one full cycle: check → data → AI → Pooja → Priya → Manager → report → verify. */
    route('POST', '/api/seo/cycle', async (ctx) => {
      if (!requireAdmin(ctx)) return;
      if (throttled(SEO_THROTTLE_KEY(ctx), 12, 60 * 60000))
        return fail(ctx.res, 429, 'Too many cycles in the last hour. The cycle is rate-limited to protect the Google API quota.');
      const result = await seo.runCycle({
        days: (ctx.body && ctx.body.days) || ctx.query.get('days') || 28,
        site: (ctx.body && ctx.body.site) || '',
        trigger: 'admin'
      });
      audit(ctx.user, 'seo.cycle', {
        target_type: 'seo_cycle',
        target_id: Number(result.cycle_id || 0),
        detail: `${result.state || 'UNKNOWN'}${result.reason ? `: ${result.reason}` : ''}`.slice(0, 300)
      });
      if (!result.ok && result.state === 'BUSY') return fail(ctx.res, 409, result.reason, { state: result.state });
      // A blocked cycle is a 200 with state=BLOCKED: the UI must show the real
      // reason instead of inventing metrics.
      return ok(ctx.res, result);
    });

    route('GET', '/api/seo/reports', async (ctx) => {
      if (!requireAdmin(ctx)) return;
      const limit = Math.min(100, Math.max(1, Number(ctx.query.get('limit') || 25)));
      const offset = Math.max(0, Number(ctx.query.get('offset') || 0));
      const reports = seo.store.listReports(limit, offset);
      return ok(ctx.res, { reports, total: seo.store.counts().reports, permanent: true });
    });

    route('GET', '/api/seo/reports/:id', async (ctx) => {
      if (!requireAdmin(ctx)) return;
      const report = seo.store.getReport(ctx.params.id);
      if (!report) return fail(ctx.res, 404, 'That report does not exist.');
      if (ctx.query.get('format') === 'md') {
        const body = report.markdown || '';
        ctx.res.writeHead(200, {
          'Content-Type': 'text/markdown; charset=utf-8',
          'Content-Disposition': `attachment; filename="seo-report-${report.cycle_no}.md"`,
          'Cache-Control': 'no-store'
        });
        ctx.res.end(body);
        return;
      }
      return ok(ctx.res, { report });
    });

    route('GET', '/api/seo/cycles', async (ctx) => {
      if (!requireAdmin(ctx)) return;
      const limit = Math.min(100, Math.max(1, Number(ctx.query.get('limit') || 25)));
      return ok(ctx.res, { cycles: seo.store.listCycles(limit), total: seo.store.counts().cycles });
    });

    route('GET', '/api/seo/cycles/:id', async (ctx) => {
      if (!requireAdmin(ctx)) return;
      const cycle = seo.store.getCycle(ctx.params.id);
      if (!cycle) return fail(ctx.res, 404, 'That cycle does not exist.');
      return ok(ctx.res, { cycle });
    });

    /** Fil One / object-storage archive state — with a real probe on demand. */
    route('GET', '/api/seo/storage', async (ctx) => {
      if (!requireAdmin(ctx)) return;
      const probe = ctx.query.get('probe') === '1';
      return ok(ctx.res, {
        archive: await seo.archiveStatus({ probe }),
        database: { kind: db.kind || 'unknown', reports: seo.store.counts().reports, cycles: seo.store.counts().cycles },
        disk: { directory: seo.store.dir, reports_directory: seo.store.reportsDir },
        tokens_encrypted: seo.store.encryption.available,
        permanent: true
      });
    });

    /** Explicit AI connectivity test — never called automatically. */
    route('POST', '/api/seo/ai/test', async (ctx) => {
      if (!requireAdmin(ctx)) return;
      const result = await seo.testAi();
      audit(ctx.user, 'seo.ai.test', {
        target_type: 'seo',
        detail: result.ok ? `${result.engine}/${result.model}` : 'no provider answered'
      });
      return ok(ctx.res, result);
    });
  }
