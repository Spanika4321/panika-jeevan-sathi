...
function createApi(options) {
  ...
  route('POST', '/api/auth/register', async (ctx) => {
    ...
    // Mirror the new member into the Google Sheet (Apps Script Web App).
    // Fire-and-forget: no await, all errors swallowed inside the connector, so
    // a Google outage can never slow down or fail a registration.
    if (gsheet.configured()) {
      Promise.resolve()
        .then(() =>
          gsheet.pushRegistration({
            user: db.one('users', { id: user.id }),
            profile: db.one('profiles', { user_id: user.id }),
            dataDir
          })
        )
        .catch(() => {});
