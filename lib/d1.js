...
const DEFAULTS = {
  ...
  // Includes SQL *and bound values*. D1-photo bridge mode stores a compressed
  // image as a ~700 KB base64 binding, so one such statement must fit while
  // still staying below a conservative 1 MB HTTP request envelope.
...
function chunkStatements(statements, limits = DEFAULTS) {
  ...
  for (const statement of statements) {
    ...
    const sqlSize = Buffer.byteLength(statement.sql, 'utf8');
    if (sqlSize > 100000) {
      throw new D1Error('Statement exceeds D1’s 100 KB SQL limit.');
    }
    // Bound values are part of the JSON request too. Counting SQL alone made a
    // batch containing photos look tiny and could create a multi-megabyte HTTP
    // request during restore. Measure the actual wire-shaped statement.
    const size = Buffer.byteLength(
      JSON.stringify({ sql: statement.sql, params }),
      'utf8'
    );
    if (size > limits.maxBytesPerRequest) {
      throw new D1Error(
        `Statement payload is ${(size / 1024).toFixed(0)} KB; the safe D1 request limit is ${(
          limits.maxBytesPerRequest / 1024
        ).toFixed(0)} KB.`
      );
    }
    const tooBig =
