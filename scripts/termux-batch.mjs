...
/**
 * The approved UI baseline file. For a batch with `ui_change_approved: true`,
 * this file is the ONLY machine record of the new public/ fingerprint — the
 * tool refuses to run until the device has captured it, so the owner's
 * "approved" is attached to a real fingerprint, not to an assumption.
 */
function baselineFile(id) {
  return path.join(BATCH_DIR, `${id}.baseline.json`);
}

function readBaseline(id) {
  return readJSON(baselineFile(id), null);
}

/**
 * Regenerate `reports/ui-baseline-body.md5` from the current public/ tree using
 * exactly the same algorithm as `scripts/health-check.mjs` (HTML <body> MD5 +
 * the five asset MD5s). This is the Guardian's design-lock file; it exists so a
 * deliberate, owner-approved UI change becomes the NEW locked baseline instead
 * of a permanent false red flag. It never touches public/**.
 */
function updateGuardianBaseline() {
  const lines = [];
  for (const f of fs.readdirSync(path.join(ROOT, 'public')).filter((n) => n.endsWith('.html')).sort()) {
  ...
  for (const f of assetFiles.sort()) {
    const hash = md5(fs.readFileSync(path.join(ROOT, 'public', f)));
    lines.push(`${hash}  public/${f}`);
  }
  const body = lines.join('\n') + '\n';
  fs.mkdirSync(path.join(ROOT, 'reports'), { recursive: true });
  fs.writeFileSync(path.join(ROOT, 'reports', 'ui-baseline-body.md5'), body);
  return body;
}
...
/**
 * Establish the new approved UI baseline ON THE DEVICE. This is the honest
 * replacement for guessing a fingerprint Arena cannot see: the device records
 * the real sha256 of the current public/ tree (which already includes the
 * owner-approved Priti AI change) and regenerates the Guardian design-lock
 * file from it. It is read-only against public/** — it never edits, renames,
 * deletes or restores a public file.
 */
async function cmdBaseline(id, { json }) {
  const { batch } = loadBatch(id);
  if (batch.ui_change_approved !== true) {
    console.error('✗ cannot establish a new UI baseline without explicit owner approval.');
    console.error(`  set ui_change_approved: true (and ui_change_reason) in ${id} first, then re-run baseline.`);
    return 1;
  }
  const executor = buildExecutor();
  const fp = publicFingerprint();
  const existing = readBaseline(id);
  const prior = batch.protected?.public_ui_fingerprint ?? null;
  const changed = !existing || existing.fingerprint !== fp.sha256;
  const baselineBody = updateGuardianBaseline();
  const record = {
    protocol: PROTOCOL,
    runner_version: RUNNER_VERSION,
    batch_id: id,
    ...
    captured_at: now(),
    approved: true,
    approved_reason: batch.ui_change_reason ?? 'owner-approved UI change',
    executor: {
      id: executor.id,
      ...
      arch: executor.arch,
      node: executor.node,
      git_head: executor.git_head,
      git_branch: executor.git_branch,
      worktree_dirty: executor.tree_dirty,
      disk_free_mb: executor.disk_free_mb,
      termux_detected: executor.termux_detected
    },
    fingerprint: fp.sha256,
    files: fp.files,
    prior_fingerprint: prior,
    prior_files: batch.protected?.public_ui_files ?? null,
    public_ui_modified_by_this_command: false,
    guardian_baseline_updated: true,
    guardian_baseline_sha256: sha256(baselineBody)
  };
  writeJSON(baselineFile(id), record);
  ...
  const out = {
    ...
    prior_files: record.prior_files,
    baseline_file: rel(baselineFile(id)),
    guardian_baseline_file: rel(path.join(ROOT, 'reports', 'ui-baseline-body.md5')),
    public_ui_modified_by_this_command: false
  };
  if (json) {
  ...
  console.log('==============================================');
  console.log(` ${id} — APPROVED UI BASELINE CAPTURED`);
  console.log('==============================================');
  console.log(`executor        : ${executor.id} (${executor.platform}/${executor.arch}, node ${executor.node})`);
  console.log(`head            : ${executor.git_head} @ ${executor.git_branch}`);
  console.log(`prior fingerprint: ${prior ? `${prior.slice(0, 16)}…` : 'none'} (${batch.protected?.public_ui_files ?? '?'} files)`);
  console.log(`new fingerprint : ${fp.sha256} (${fp.files} files)`);
  console.log(`changed?        : ${changed ? 'YES — this is the approved change' : 'no change since last capture'}`);
  console.log(`baseline file   : ${rel(baselineFile(id))}`);
  console.log(`guardian lock   : ${rel(path.join(ROOT, 'reports', 'ui-baseline-body.md5'))} regenerated`);
  console.log('public/**       : untouched — nothing under public/ was modified by this command');
  console.log('==============================================');
  return 0;
}
...
async function cmdRun(id, opts) {
  ...
  /* An approved UI change is only machine-enforced once the device has
     captured a real fingerprint for it. Refuse to run on a guess. */
  if (batch.ui_change_approved === true) {
    const cap = readBaseline(id);
    if (!cap) {
      console.error('BATCH REJECTED BEFORE EXECUTION: ui_change_approved is true but no baseline has been captured.');
      console.error(`  run first (read-only on public/**, writes only ops/batches/ + reports/):`);
      console.error(`    node scripts/termux-batch.mjs baseline ${id}`);
      return 1;
    }
    if (executor.public_ui_fingerprint !== cap.fingerprint) {
      console.error('BATCH REJECTED BEFORE EXECUTION: the working tree does not match the captured baseline.');
      console.error(`  captured fingerprint ${cap.fingerprint.slice(0, 16)}… (${cap.files} files)`);
      console.error(`  current  fingerprint ${executor.public_ui_fingerprint.slice(0, 16)}… (${executor.public_ui_files} files)`);
      console.error('  if the tree intentionally changed again, re-capture the baseline (or restore the reported state).');
      return 1;
    }
  }
  ...
  /* UI preservation guard: public/** must be byte-identical to what it was at
     the START of this run. An approved UI change is captured before the run
     (baseline), so the start fingerprint already includes it; any further
     change DURING the batch is still a violation and is never forgiven. */
  const uiDrift = publicFingerprint().sha256 !== executor.public_ui_fingerprint;
  if (uiDrift) {
    console.log('\n  ✗ PUBLIC UI DRIFT DETECTED DURING BATCH — public/** changed while the batch was running.');
  }
...
function cmdValidate(id, opts) {
  ...
  if (batch.ui_change_approved === true) {
    if (drift) {
      violations.push('public UI moved DURING the batch — an approved change may be captured once, before the run, and must not change again while the batch is running');
    }
    if (!baseline) {
      violations.push('ui_change_approved is true but no captured baseline exists (ops/batches/<BATCH>.baseline.json) — run `termux-batch.mjs baseline <BATCH>` on the device first');
    } else {
      if (!res.public_ui_fingerprint_before || res.public_ui_fingerprint_before !== baseline.fingerprint) {
        violations.push(`result does not start on the captured baseline (${String(res.public_ui_fingerprint_before).slice(0, 12)}… vs ${baseline.fingerprint.slice(0, 12)}…)`);
      }
      if (!res.public_ui_fingerprint_after || res.public_ui_fingerprint_after !== baseline.fingerprint) {
        violations.push(`result does not end on the captured baseline (${String(res.public_ui_fingerprint_after).slice(0, 12)}… vs ${baseline.fingerprint.slice(0, 12)}…)`);
      }
    }
  } else if (drift) {
    violations.push('public UI changed during the batch without explicit approval — unauthorized design change');
  } else if (baseline && res.public_ui_fingerprint_after && res.public_ui_fingerprint_after !== baseline.fingerprint) {
    violations.push(`result ends on a different fingerprint than the captured baseline (${String(res.public_ui_fingerprint_after).slice(0, 12)}… vs ${baseline.fingerprint.slice(0, 12)}…)`);
  }
  const counts = {
...
function usage() {
  ...
  baseline <BATCH> [--json]             capture the approved UI baseline on the device (read-only vs public/**)
...
async function main() {
  ...
  switch (command) {
    ...
    case 'baseline':
      return id ? cmdBaseline(id, { json: has('--json') }) : usage();
    case 'run':
      return id
        ? cmdRun(id, {
