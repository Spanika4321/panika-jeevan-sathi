#!/usr/bin/env node
/**
 * PANIKA JEEVAN SATHI — durable storage connection check.
 *
 * Run this after filling in the D1 environment variables (locally, on Render's
 * "Shell"/one-off job, or from GitHub Actions). R2 is checked only when it is
 * configured; otherwise photos use the supported D1 bridge.
 *
 *   node scripts/verify-cloud.mjs              # check D1 (+ optional R2)
...
if (!d1Config) {
  ...
  try {
    ...
    const expected = ['users', 'profiles', 'interests', 'shortlist', 'messages', 'notifications', 'photo_blobs'];
...
section('2. Profile photo backend');

const r2Config = r2Lib.configFromEnv();
if (!r2Config) {
  check('R2 is optional; D1 photo bridge is selected', Boolean(d1Config), 'max 512 KB/photo');
} else {
...
if (urlArg) {
  ...
  try {
    ...
    const photoStats = body && body.remote && body.remote.photos;
    check(
      'photos are durable in D1 or R2',
      photoStats && photoStats.remote === true && ['d1', 'r2'].includes(photoStats.backend),
      photoStats ? `backend=${photoStats.backend}` : ''
    );
